...
export default async function handler(req, res) {
  ...
  try {
    ...
    const countries = rawCountries.map(country => {
      const id = country.id ?? country.country_id ?? country.countryId;
      const name = country.name ?? country.country_name ?? country.countryName ?? String(id);
      const code = country.code ?? country.country_code ?? country.countryCode ?? "";
      const server =
        String(code).toUpperCase() === "US" ||
        String(name).toLowerCase() === "united states" ||
        String(name).toLowerCase() === "united states of america"
          ? "usa-server-1"
          : "global-server-1";

      return { ...country, id, name, code, server };
    });
