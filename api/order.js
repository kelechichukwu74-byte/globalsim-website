...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Connect every provider portal that can serve this country.
    // US: Server 2 first, then Server 1, then the two global portals.
    // Other countries: both global portals.
    // If the service catalog supplied a list of portals, try those first
    // while still keeping the correct fallback portals.
    const countryForRouting =
      body.countryCode ||
      countryName ||
      countryId;

    const defaultServers =
      getServersForCountry(countryForRouting);

    const requestedServers =
      Array.isArray(body.availableServers)
    ...
    for (const server of servers) {
      try {
        ...
        if (candidateVerificationId && candidatePhoneNumber) {
          ...
          providerPrice = candidateProviderPrice;
        ...
        providerError = new Error(
          `${server}: ${error?.message || "provider request failed"}`
        );
        console.error(
      ...
      await createOrder(
        user.id,
        {
          ...
          status:
            verification?.status ||
            "active",
          provider_server: selectedServer
        }
      );
