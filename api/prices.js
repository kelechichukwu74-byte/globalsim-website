  ...
  try {
    ...
    for (const server of servers) {
      try {
        ...
        if (Number.isFinite(price) && price > 0) results.push({ server, price, providerPrice: price, available:true, raw:data });
      } catch (e) { results.push({ server, available:false, providerPrice: null, error:e?.message || "Unavailable" }); }
    }
